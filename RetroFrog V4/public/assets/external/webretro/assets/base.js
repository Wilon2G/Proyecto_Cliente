...
var biosCdn =
  'https://cdn.jsdelivr.net/gh/archtaurus/RetroPieBIOS@master/BIOS/';
var infoJsonUrl =
  'https://cdn.jsdelivr.net/gh/BinBashBanana/webretro/assets/info.json';
var standaloneDownloadUrl =
  'https://cdn.jsdelivr.net/gh/BinBashBanana/webretro/utils/webretro-standalone.html';
var relativeBase = typeof relativeBase == 'string' ? relativeBase : '';
var coreDir = 'cores/';
var bioses = {
  a5200: { path: '', files: ['5200.rom'] },
  atari800: { path: '', files: ['5200.rom'] },
  freechaf: { path: '', files: ['sl31253.bin', 'sl31254.bin', 'sl90025.bin'] },
  freeintv: { path: '', files: ['exec.bin', 'grom.bin'] },
  gearcoleco: { path: '', files: ['colecovision.rom'] },
  handy: { path: '', files: ['lynxboot.img'] },
  mednafen_psx: {
    path: '',
  ...
  neocd: { path: 'neocd/', files: ['neocd_z.rom'] },
  o2em: { path: '', files: ['c52.bin', 'g7400.bin', 'jopac.bin', 'o2rom.bin'] },
  opera: {
    path: '',
  ...
  yabasanshiro: { path: '', files: ['saturn_bios.bin'] },
  yabause: { path: '', files: ['saturn_bios.bin'] },
};
var defaultKeybinds =
...
var downloadStandaloneButton = document.getElementById(
...
var themes = {
  iodinelight: { menuHeight: 45, id: '' },
  iodinedark: { menuHeight: 45, id: 'iodinedark' },
  webplayer: { menuHeight: 65, id: 'webplayer' },
  webplayernavy: { menuHeight: 65, id: 'webplayer navy' },
};
var defaultTheme = window.matchMedia('(prefers-color-scheme: dark)').matches
  ? 'iodinedark'
  : 'iodinelight';
...
var coreOptions = {
  a5200: { lowpass: 'a5200_low_pass_filter = "enabled"\n' },
  mednafen_psx: {
    highres: 'beetle_psx_internal_resolution = "2x"\n',
  ...
  mednafen_psx_hw: {
    softwarerenderer: 'beetle_psx_hw_renderer = "software"\n',
  ...
  parallel_n64: {
    highres: 'parallel-n64-screensize = "960x720"\n',
...
var fileExts = {
  ...
  ColecoVision: '.col, .cv',
  'Commodore 64':
    '.d64, .d6z, .d71, .d7z, .d80, .d81, .d82, .d8z, .g64, .g6z, .g41, .g4z, .x64, .x6z, .nib, .nbz, .d2m, .d4m, .t64, .p00',
  'Electronika BK': '',
...
var cdromCores = [
  ...
  'yabasanshiro',
  'kronos',
]; // probably put these in playlistCores too
var multiSaveCores = [
  'pcsx2',
  'mednafen_psx',
  'mednafen_psx_hw',
...
var allFileExts = Array.from(
  new Set(
    Object.values(fileExts)
      .filter((i) => i)
      .join(', ')
      .split(', '),
  ),
).join(', ');
var systemsExperimentalFormat = Object.fromEntries(
  Object.values(systems).map((i) => [
    i,
    allCores.filter((j) => systems[j] == i),
  ]),
);
var installedCores = [
  'a5200',
...
var installedSystems = allSystems.filter((i) =>
  installedCores.some((j) =>
    allCores.filter((k) => systems[k] == i).includes(j),
  ),
);
var installedFileExts = installedSystems
  .map((i) => fileExts[i])
  .filter((i) => i)
  .join(', ');
var coreGithub = document.getElementById('coregithub');
var coreGithubLinks = {
  81: 'libretro/81-libretro',
  dosbox: 'libretro/dosbox-libretro',
  dosbox_pure: 'schellingb/dosbox-pure',
  opera: 'libretro/opera-libretro',
  fsuae: 'libretro/libretro-fsuae',
  puae: 'libretro/libretro-uae',
  cap32: 'libretro/libretro-cap32',
  fbalpha2012: 'libretro/fbalpha2012',
  fbneo: 'libretro/FBNeo',
  mame2003_plus: 'libretro/mame2003-plus-libretro',
  stella: 'stella-emu/stella',
  stella2014: 'libretro/stella2014-libretro',
  atari800: 'libretro/libretro-atari800',
  a5200: 'libretro/a5200',
  prosystem: 'libretro/prosystem-libretro',
  virtualjaguar: 'libretro/virtualjaguar-libretro',
  handy: 'libretro/libretro-handy',
  mednafen_lynx: 'libretro/beetle-lynx-libretro',
  hatari: 'libretro/hatari',
  gearcoleco: 'drhelius/Gearcoleco',
  vice_x64: 'libretro/vice-libretro',
  bk: 'libretro/bk-emulator',
  freechaf: 'libretro/FreeChaF',
  gw: 'libretro/gw-libretro',
  vba_next: 'libretro/vba-next',
  vbam: 'libretro/vbam-libretro',
  meteor: 'libretro/meteor-libretro',
  mednafen_gba: 'libretro/beetle-gba-libretro',
  gpsp: 'libretro/gpsp',
  mgba: 'libretro/mgba',
  gambatte: 'libretro/gambatte-libretro',
  gearboy: 'drhelius/Gearboy',
  tgbdual: 'libretro/tgbdual-libretro',
  sameboy: 'libretro/SameBoy',
  o2em: 'libretro/libretro-o2em',
  freeintv: 'libretro/FreeIntv',
  fmsx: 'libretro/fmsx-libretro',
  bluemsx: 'libretro/blueMSX-libretro',
  neocd: 'libretro/neocd_libretro',
  mednafen_ngp: 'libretro/beetle-ngp-libretro',
  bnes: 'libretro/bnes-libretro',
  fceumm: 'libretro/libretro-fceumm',
  quicknes: 'libretro/QuickNES_Core',
  mesen: 'libretro/Mesen',
  nestopia: 'libretro/nestopia',
  citra: 'libretro/citra',
  mupen64plus_next: 'libretro/mupen64plus-libretro-nx',
  parallel_n64: 'libretro/parallel-n64',
  desmume2015: 'libretro/desmume2015',
  ...
  mednafen_supergrafx: 'libretro/beetle-supergrafx-libretro',
  quasi88: 'libretro/quasi88-libretro',
  np2kai: 'AZO234/NP2kai',
  mednafen_pcfx: 'libretro/beetle-pcfx-libretro',
  mednafen_psx: 'libretro/beetle-psx-libretro',
  mednafen_psx_hw: 'libretro/beetle-psx-libretro',
  pcsx2: 'libretro/pcsx2',
  ppsspp: 'hrydgard/ppsspp',
  scummvm: 'libretro/scummvm',
  flycast: 'flyinghead/flycast',
  mednafen_saturn: 'libretro/beetle-saturn-libretro',
  mednafen_saturn_hw: 'libretro/beetle-saturn-libretro',
  yabause: 'libretro/yabause',
  yabasanshiro: 'libretro/yabause/tree/yabasanshiro',
  kronos: 'libretro/yabause/tree/kronos',
  ...
  fuse: 'libretro/fuse-libretro',
  bsnes: 'libretro/bsnes-libretro',
  bsnes_mercury_performance: 'libretro/bsnes-mercury',
  bsnes_mercury_balanced: 'libretro/bsnes-mercury',
  bsnes_mercury_accuracy: 'libretro/bsnes-mercury',
  mednafen_snes: 'libretro/beetle-bsnes-libretro',
  'mesen-s': 'libretro/Mesen-S',
  snes9x: 'libretro/snes9x',
  theodore: 'Zlika/theodore',
  vecx: 'libretro/libretro-vecx',
  mednafen_vb: 'libretro/beetle-vb-libretro',
  mednafen_wswan: 'libretro/beetle-wswan-libretro',
};
...
var queries = Object.fromEntries(
  window.location.search
    .substring(1)
    .split('&')
    .map((i) => i.split('='))
    .map((i) => i.map((i) => i && decodeURIComponent(i))),
);
...
function configIDToCode(configid) {
  return Object.keys(codeToConfigIDMap).find(
    (k) => codeToConfigIDMap[k] == configid,
  );
}
...
function setIdbItem(key, value, customTransaction) {
  (customTransaction || wIdb.transaction('main', 'readwrite'))
    .objectStore('main')
                ...
                [
                  {
                    ...
                    dir: '',
                    data: new Uint8Array(
                      JSON.parse(localStorage.getItem(ls[i])),
                    ),
                  },
                ],
                transaction,
        ...
        (async function () {
          ...
          for (var i = 0; i < allItems.length; i++) {
            if (allItems[i].key.startsWith('RetroArch_saves_')) {
              setIdbItem(
                allItems[i].key,
                [{ ext: '.srm', dir: '', data: allItems[i].value }],
                transaction,
              );
            }
          }
...
function log(log, userInput) {
  console.log(log);
  if (maxConsoleLength > 0 && wconsole.textContent.length > maxConsoleLength)
    wconsole.textContent = wconsole.textContent.substring(
      wconsole.textContent.indexOf('\n') + 1,
    );
...
function uploadFileMulti(accept, callback) {
  ...
  input.onchange = async function () {
    ...
    for (var i = 0; i < this.files.length; i++) {
      var name = directoryUpload
        ? (this.files[i].relativePath || this.files[i].webkitRelativePath || '')
            .split('/')
            .slice(1)
            .join('/')
        : this.files[i].name;
...
function downloadStandaloneWebretro() {
  grab(
    ...
    function (data) {
      alert(
        'Downloading standalone webretro. Internet is required for operation.',
      );
      downloadFile(
        data,
        standaloneDownloadUrl.split('/').slice(-1)[0],
        'text/html',
      );
    },
    function () {
...
function getFileExtsForCore(core) {
  return [
    ...
    cdromCores.includes(core) ? cdromExts : '',
    playlistCores.includes(core) ? playlistExts : '',
  ]
    .filter((i) => i)
    .join(', ');
}
...
function unzipFile(data, exts, callback, empty, notfound) {
  new zip.ZipReader(new zip.Uint8ArrayReader(new Uint8Array(data)))
    .getEntries()
        ...
        for (var i = 0; i < entries.length; i++) {
          if (
            ...
            exts
              ...
              .includes(
                '.' +
                  u8atoutf8(entries[i].rawFilename)
                    .split('.')
                    .slice(-1)[0]
                    .toLowerCase(),
              )
...
async function zipFiles(files, callback) {
  ...
  for (var i = 0; i < files.length; i++) {
    await writer.add(
      files[i].path,
      new zip.Uint8ArrayReader(new Uint8Array(files[i].data)),
    );
  }
  await writer.close();
...
function readyRomUploads(exts) {
  ...
  window.fileDropped = async function (e) {
    if (e.dataTransfer.types.includes('Files')) {
      ...
      let fileTree = Array.from(e.dataTransfer.items).map((i) =>
        i.webkitGetAsEntry(),
      );
...
wconsole.wconsoleinput.onkeydown = async function (e) {
  ...
  if (e.isTrusted && e.code == 'Enter') {
    log(
      await eval('(async function() { return ' + this.value + ' })()'),
      this.value,
    );
    this.value = '';
  }
...
var savedKeybindsObj = localStorage.getItem('RetroArch_settings_keybinds')
  ? Object.assign(
      Object.assign({}, defaultKeybindsObj),
      configStrToObj(localStorage.getItem('RetroArch_settings_keybinds')),
    )
  : Object.assign({}, defaultKeybindsObj);
...
function createConfigList() {
  ...
  for (var i = 0; i < validKeybinds.length; i++) {
    keybindTable.innerHTML +=
      '<tr><td>' +
      validKeybinds[i].replace(/^input_/, '') +
      '</td><td>' +
    ...
    switch (e.target.dataset.action) {
      case 'download':
        downloadFile(
          ...
          screenshotDatas[screenshotNo].path.replace('rom', romName),
          'image/png',
        );
        break;
      case 'delete':
        if (confirm('Are you sure you want to delete this screenshot?')) {
          ...
          FS.unlink(
            '/home/web_user/retroarch/userdata/screenshots/' +
              screenshotDatas[screenshotNo].path,
          );
      ...
      case 'download':
        getIdbItem(saveIDs[saveNo].id).then(function (data) {
          if (saveIDs[saveNo].type == 'save') {
            ...
            if (files.length == 1) {
              downloadFile(
                ...
                'game-sram-' +
                  ...
                  getTime() +
                  '.' +
                  files[0].path.split('.').slice(1).join('.'),
              );
            } else {
            ...
            downloadFile(
              data,
              'game-state-' + saveIDs[saveNo].name + '-' + getTime() + '.state',
            );
          }
        });
        break;
...
function getMasterRom(files) {
  return new Promise(function (resolve) {
    ...
    if (recommendedExts) {
      ...
      var detectedFiles = files.filter((i) =>
        recommendedExtsArray.includes(
          '.' + i.path.toLowerCase().split('.').slice(-1)[0],
        ),
      );
    ...
    if (recommendedExts) {
      recommendedExtensions.textContent =
        'Recommended file extensions: ' + recommendedExts;
    } else {
    ...
    romSelectorTable.onclick = function (e) {
      closeManagers();
      resolve(
        Array.from(romSelectorTable.children).indexOf(e.target.parentElement),
      );
      return;
    };
  });
...
(function () {
  ...
  if (!queries.core && queries.system) {
    var detectedCores = allCores.filter(
      (k) => systems[k].toLowerCase() == queries.system.toLowerCase(),
    );
    var usableCores = installedCores.filter(
      (k) => systems[k].toLowerCase() == queries.system.toLowerCase(),
    );
    var usingCore =
      usableCores.find((k) => preferredCores.includes(k)) || usableCores[0];
    if (usingCore) {
      ...
      alert(
        'Could not find any cores matching the system "' +
          queries.system +
          '".',
      );
    } else {
      alert(
        'Found the core(s) ' +
          detectedCores.join(', ') +
          ', but none were marked as installed.',
      );
    }
  }
      ...
      getCore(
        ...
        function () {
          ...
          if (romMode != 'querystring')
            document.title =
              (coreNames[core] || core) + (appIsPwa ? '' : ' | webretro');

          readyRomUploads(
            [
              '.zip' + (exclusiveMultiFileCores.includes(core) ? '' : ', .bin'),
              allCores.includes(core) ? getFileExtsForCore(core) : allFileExts,
            ]
              .filter((i) => i)
              .join(', '),
          );
...
function initFromFile(files) {
  if (
    files.length == 1 &&
    files[0].path.split('.').slice(-1)[0].toLowerCase() == 'zip'
  ) {
    if (multiFileCores.includes(core)) {
      log(
        'Zip file detected, unzipping... (multi-file ROM detected... probably)',
      );
      ...
      unzipFile(
        files[0].data,
        ['.bin', getFileExtsForCore(core)].filter((i) => i).join(', '),
...
function autodetectCore(name, data) {
  ...
  var detectedSystem = allSystems.find((k) =>
    fileExts[k].split(', ').includes(nameExt),
  );
  ...
  if (usingCore) {
    ...
    alert(
      'Unrecognized file extension "' +
        nameExt +
        '". This does not mean that it is unsupported, it may just mean that it is not auto-detectable.',
    );
  } else {
    alert(
      'Found the core(s) ' +
...
function readyForInit(files) {
  ...
  if (
    romMode == 'querystring' &&
    (queries.hasOwnProperty('forcestartbutton') || !isAudioAllowed())
  ) {
...
function prepareBios() {
  if (bioses[core]) {
    ...
    for (let i = 0; i < bios.files.length; i++) {
      grab(
        biosCdn + bios.files[i],
        'arraybuffer',
        function (data) {
          FS.writeFile(
            baseFsSystemDir + bios.path + bios.files[i],
            new Uint8Array(data),
          );
...
function doNotRename() {
  if (
    romMode == 'upload' &&
    !localStorage.getItem('webretro_settings_pastFirstSave')
  ) {
    alert(
      'WARNING: Do not rename your ROM file after this! The save data is specific to the ROM file name!',
    );
...
function saveSRAMHandler(path) {
  saveObj[path.replace(baseFsSaveDir, '').replace('rom', 'ROMNAME')] =
    FS.readFile(path);
...
function writeToFileHandler(path) {
  ...
  if (path.startsWith(baseFsSaveDir)) {
    ...
    if (path == '/home/web_user/retroarch/userdata/states/rom.state')
      saveStateHandler();
  }
}
...
function afterStart() {
      ...
      if (multiSaveCores.includes(core)) {
        uploadFileMulti('.zip, .bin, ' + sramExts, function (files) {
          if (files.length == 1) {
              ...
              unzipFileMulti(
                ...
                function (uzfiles) {
                  setIdbItem(
                    ...
                    saveFilesToArr(replaceInFiles(uzfiles, romName, 'ROMNAME')),
                  );
                  done();
                },
                function () {
            ...
            setIdbItem(
              'RetroArch_saves_' + romName,
              saveFilesToArr(replaceInFiles(files, romName, 'ROMNAME')),
            );
            done();
          }
        });
  ...
  exportSave.onclick = function () {
    var files = replaceInFiles(
      saveArrToFiles(saveObjToArr(saveObj)),
      'ROMNAME',
      romName,
    );
    if (!files.length) {
  ...
  async function waitForReady() {
    if (wasmReady && bundleReady && biosReady) {
      ...
      log(
        data.length == 1
          ? 'Initializing with ' +
              bytesToHumanReadable(data[0].data.byteLength) +
              ' of data'
          : 'Initializing with multiple files',
      );
      ...
      if (data.length == 1) {
        ...
        if (
          exclusiveMultiFileCores.includes(core) &&
          confirm(
            'Rename similar files? (Use if you get "Unable to find rom" errors. Otherwise don\'t use.)',
          )
        ) {
          for (var i = 0; i < data.length; i++) {
            if (!data[i].path.includes('/'))
              data[i].path = data[i].path.replace(romName, 'rom');
          }
        }

        FS.createPath('/', 'rom', true, true);
        var parentDirs = Array.from(
          new Set(data.map((i) => i.path.split('/').slice(0, -1).join('/'))),
        ).filter((i) => i);
      ...
      if (cSave) {
        ...
        for (var i = 0; i < cSave.length; i++) {
          safeWriteFile(
            baseFsSaveDir + cSave[i].dir + 'rom' + cSave[i].ext,
            cSave[i].data,
          );
        }
      ...
      if (cState) {
        ...
        new sideAlert(
          'State imported for ' + romName + ' (press load state)',
          5000,
        );
      ...
      if (coreOptions[core]) {
        ...
        try {
          ...
          for (var i = 0; i < opts.length; i++) {
            if (opts[i].checked && coreOptions[core][opts[i].dataset.opt])
              coreOptionsString += coreOptions[core][opts[i].dataset.opt];
          }
        } catch (e) {
          console.warn(e);
        }
      ...
      switch (core) {
        ...
        case 'mednafen_psx':
          safeWriteFile(
            baseFsConfigDir + 'Beetle PSX/Beetle PSX.opt',
            coreOptionsString,
          );
          break;
        case 'mednafen_psx_hw':
          safeWriteFile(
            baseFsConfigDir + 'Beetle PSX HW/Beetle PSX HW.opt',
            coreOptionsString,
          );
          break;
        case 'mednafen_vb':
          safeWriteFile(
            baseFsConfigDir + 'Beetle VB/Beetle VB.opt',
            coreOptionsString,
          );
          break;
        case 'mednafen_wswan':
          safeWriteFile(
            baseFsConfigDir + 'Beetle WonderSwan/Beetle WonderSwan.opt',
            coreOptionsString,
          );
          break;
        case 'melonds':
          safeWriteFile(
            baseFsConfigDir + 'melonDS/melonDS.opt',
            coreOptionsString + 'melonds_touch_mode = "Touch"\n',
          );
          break;
        case 'mgba':
        ...
        case 'mupen64plus_next':
          safeWriteFile(
            baseFsConfigDir + 'Mupen64Plus-Next/Mupen64Plus-Next.opt',
            coreOptionsString +
        ...
        case 'parallel_n64':
          safeWriteFile(
            baseFsConfigDir + 'ParaLLEl N64/ParaLLEl N64.opt',
            coreOptionsString,
          );
          break;
        case 'prosystem':
          safeWriteFile(
            baseFsConfigDir + 'ProSystem/ProSystem.opt',
            coreOptionsString,
          );
          break;
        case 'snes9x': // actually a remap
          safeWriteFile(
            baseFsConfigDir + 'remaps/Snes9x/Snes9x.rmp',
            coreOptionsString,
          );
          break;
        case 'stella2014':
          safeWriteFile(
            baseFsConfigDir + 'Stella 2014/Stella 2014.opt',
            coreOptionsString,
          );
          break;
        case 'vecx':
        ...
        case 'virtualjaguar':
          safeWriteFile(
            baseFsConfigDir + 'Virtual Jaguar/Virtual Jaguar.opt',
            coreOptionsString + 'virtualjaguar_bios = "enabled"\n',
          );
          break;
        case 'yabause':
          safeWriteFile(
            baseFsConfigDir + 'Yabause/Yabause.opt',
            coreOptionsString,
          );
          break;
      }
      ...
      FS.trackingDelegate.onWriteToFile = function (path) {
        if (!path.startsWith('/dev/')) {
          if (writeToFileCooldown[path])
            window.clearTimeout(writeToFileCooldown[path]);
          writeToFileCooldown[path] = window.setTimeout(function () {
            delete writeToFileCooldown[path];
            FSTracking.dispatchEvent(
              new CustomEvent('writeToFile', { detail: path }),
            );
      ...
      try {
        ...
        var estr =
          'FAILED TO CALL MAIN. CHECK BROWSER CONSOLE FOR DETAILS. (core: ' +
          core +
          ')';
        alert(estr);
        log(estr);
